...
public class MobileController {
    ...
    public
    @ResponseBody
    String getAllPMobileBrands(@PathVariable("category") String category) {
        System.out.println("Category is :: " + category);
        String result = "";
        try {
            Set<String> finalBrandlist = dataServices.getAllMobileBrandList(category);
    ...
    public
    @ResponseBody
    String getAllPMobileColor(@PathVariable("category") String category) {
        System.out.println("Category is :: " + category);
        String result = "";
        try {
            Set<String> finalBrandlist = dataServices.getAllMobileColorList(category);
            List<Brand> brnBrand = new ArrayList<Brand>();
            for (String brand1 : finalBrandlist) {
                Brand brand11 = new Brand();
    ...
    String getAllSelect(@PathVariable("brand") String brand, @PathVariable("color") String color, @PathVariable("price") String price,
        ...
        try {
            List<PMobile> finalProd = dataServices.getAllMobileList(brand, color, maxPrice, minPrice, categoryItem, order);
    ...
    public
    @ResponseBody
    String getAllPMobileClothsForCat(@PathVariable("category") String category) {

        String result = "";
        try {
            List<PMobile> finalBrandlist = dataServices.getMobileForCatList(category);

            if (category.equalsIgnoreCase("LED") || category.equalsIgnoreCase("LCD") || category.equalsIgnoreCase("Plasma") ) {
                for (Iterator<PMobile> iter = finalBrandlist.iterator(); iter.hasNext(); ) {
                    PMobile p = iter.next();
            ...
            IndexWriter w = new IndexWriter(index, config);
            for (PMobile product : finalBrandlist) {
                addDoc(w, product.getProductName(), product.getSalePrice(), product.getMerchant(), product.getProductURL(),
                        product.getMediumImage());
            }
            w.close();
    ...
    String compareProduct(@PathVariable("productName") String productName, @PathVariable("category") String category) {
        ...
        String result = "";
        try {
            List<PMobile> PMobiles = dataServices.getMobileForCatList(category);
