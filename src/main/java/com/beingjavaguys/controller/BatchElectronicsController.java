...
public class BatchElectronicsController {
    ...
    @RequestMapping(value = "/electronics", method = RequestMethod.POST, consumes = MediaType.ALL_VALUE)
    public
    @ResponseBody
    Status addMobile() throws Exception {
        System.out.println("Calling Menclothing Services");

        List<PElectronics> finalList = new ArrayList<>();

        JSONObject jsonObject = null;
        String str = "http://feeds.omgeu.com/data/feed.aspx?hash=45916129896542018acd3eea48b27d4b" + "&page=1";
        URL url = new URL(str);
        ...
        for (int i = 1; i < 10; i++) {
            List<PElectronics> list = new ArrayList<>();
        ...
        for (PElectronics product : finalList) {
            if (product.getMediumImage()!=null) {
                dataServices.addPElectronicsEntity(product);
                Thread.sleep(50);
            }
        }

        // List<PElectronics> fliPElectronicsList = flipKartMobileData();

        //  List<PElectronics> amazonResults = getAmazonResults();

        // List<PElectronics> snapAndInfiResults = getSnapdealAndInfibeamResults();

        //   System.out.println("omgMobileDataList size : " + omgMobileDataList.size());

     /*   for (PElectronics product : omgMobileDataList) {
...
*/
        /*for (PElectronics product : fliPElectronicsList) {
       ...
       /* for (PElectronics product : amazonResults) {
...
*/
      /*  for (PElectronics product : snapAndInfiResults) {
            dataServices.addPElectronicsEntity(product);
            System.out.println("tshirtcreate Product added Successfully !");

        }*/

        return new Status(1, "Menclothing Product added Successfully !");

    }

    public List<PElectronics> omgMobileData(int pageNo) throws IOException, JSONException, org.json.JSONException {

        JSONObject jsonObject = null;
        String str = "http://feeds.omgeu.com/data/feed.aspx?hash=45916129896542018acd3eea48b27d4b" + "&page=" + pageNo;
        URL url = new URL(str);
        InputStream is = url.openStream();
        int ptr = 0;
        StringBuilder builder = new StringBuilder();
        ...
        List<PElectronics> productList = new ArrayList<PElectronics>();
        for (int i = 0; i < productList1.length(); i++) {

            PElectronics pinfo = new PElectronics();
    ...
    public List<PElectronics> flipKartMobileData() throws IOException, JSONException, org.json.JSONException {

        List<PElectronics> productList = new ArrayList<PElectronics>();

        HeaderColumnNameTranslateMappingStrategy<PElectronics> beanStrategy = new HeaderColumnNameTranslateMappingStrategy<PElectronics>();
        beanStrategy.setType(PElectronics.class);
        ...
        CsvToBean<PElectronics> csvToBean = new CsvToBean<PElectronics>();
        CSVReader reader = new CSVReader(new FileReader("C:\\pingleCSV\\MenClothing\\MenClothing8Oct.csv"));
        List<PElectronics> emps = csvToBean.parse(beanStrategy, reader);
        // System.out.println(emps.size());
        for (PElectronics product : emps) {
  ...
  /*  public List<PElectronics> getAmazonResults() {
        // Set the service:
        ...
        List<PElectronics> products = new ArrayList<PElectronics>();
        for (int i = 1; i < 10; i++) {
            ...
            for (Items item : response.getItems()) {
                ...
                for (Item item1 : item.getItem()) {
                    PElectronics product = new PElectronics();
...
*/
    ...
    /*public List<PElectronics> getSnapdealAndInfibeamResults() throws FileNotFoundException {

        List<PElectronics> productList = new ArrayList<PElectronics>();

        HeaderColumnNameTranslateMappingStrategy<PElectronics> beanStrategys = new HeaderColumnNameTranslateMappingStrategy<PElectronics>();
        beanStrategys.setType(PElectronics.class);
        ...
        CsvToBean<PElectronics> csvToBeans = new CsvToBean<PElectronics>();
        CSVReader readers = new CSVReader(new FileReader("C:\\pingleCSV\\Laptops\\SnapdealLaptops.csv"));
        List<PElectronics> empss = csvToBeans.parse(beanStrategys, readers);
        System.out.println("omg list size = " + empss.size());
        for (PElectronics product : empss) {
            // System.out.println("Name - " + product.getProductName() + "Price - " + product.getSalePrice() + " - color - " + product.getColor());
            List<String> items = Arrays.asList(product.getMediumImage().split("\\s*,\\s*"));
            product.setMediumImage(items.get(0));
      ...
      *//*  HeaderColumnNameTranslateMappingStrategy<PElectronics> beanStrategy2 = new HeaderColumnNameTranslateMappingStrategy<PElectronics>();
        beanStrategy2.setType(PElectronics.class);
        ...
        CsvToBean<PElectronics> csvToBean2 = new CsvToBean<PElectronics>();
        CSVReader reader2 = new CSVReader(new FileReader("C:\\pingleCSV\\InfibeamMobilesFeed.csv"));
        List<PElectronics> emps2 = csvToBean2.parse(beanStrategy2, reader2);
        System.out.println("omg list size = " + emps2.size());
        for (PElectronics product : emps2) {
            // System.out.println("Name - " + product.getProductName() + "Price - " + product.getSalePrice() + " - color - " + product.getColor());
