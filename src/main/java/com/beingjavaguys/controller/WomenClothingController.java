...
public class WomenClothingController {
    ...
    String getAllSelectClothing(@PathVariable("brand") String brand, @PathVariable("color") String color, @PathVariable("price") String price,
        ...
        try {
            List<PWomenClothing> finalProd = dataServices.getAllWomenSelectClothingList(brand, color, maxPrice, minPrice, categoryItem, order);

            if (categoryItem.equalsIgnoreCase("Dress")) {
                for (Iterator<PWomenClothing> iter = finalProd.iterator(); iter.hasNext(); ) {
    ...
    String compareProduct(@PathVariable("productName") String productName, @PathVariable("category") String category) {
        ...
        String result = "";
        try {
            List<PWomenClothing> pWomenClothings = dataServices.getWomenClothingForCatList(category);
            ...
            for (PWomenClothing product : pWomenClothings) {
            ...
            List<PWomenClothing> productList1 = new ArrayList<PWomenClothing>();

            //  System.out.println("productList before searching = " + productList.size());
            //  System.out.println("Found " + hits.length + " hits.");
            for (int i = 0; i < hits.length; ++i) {
                int docId = hits[i].doc;
                PWomenClothing product45 = new PWomenClothing();
