...
public class BatchWomenClothingController {
    ...
    @RequestMapping(value = "/womenclothing", method = RequestMethod.POST, consumes = MediaType.ALL_VALUE)
    public
    @ResponseBody
    Status addMobile() throws Exception {
        System.out.println("Calling womenclothing Services");

        List<PWomenClothing> finalList = new ArrayList<>();
        ...
        for (int i = 1; i < 10; i++) {
            List<PWomenClothing> list = new ArrayList<>();
        ...
        for (PWomenClothing product : finalList) {
            dataServices.addPWomenClothingEntity(product);
...
*/
      /*  for (PTshirt product : snapAndInfiResults) {
        ...
        return new Status(1, "womenclothing Product added Successfully !");

    }

    public List<PWomenClothing> omgMobileData(int pageNo) throws IOException, JSONException, org.json.JSONException {
        ...
        List<PWomenClothing> productList = new ArrayList<PWomenClothing>();
        for (int i = 0; i < productList1.length(); i++) {

            PWomenClothing pinfo = new PWomenClothing();
