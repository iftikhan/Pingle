...
import com.beingjavaguys.model.PCamera;
...
public class BatchCameraController {
    ...
    @RequestMapping(value = "/cameracreate", method = RequestMethod.POST, consumes = MediaType.ALL_VALUE)
    public
    @ResponseBody
    Status addMobile() throws Exception {
        System.out.println("Calling cameracreate Services");

        List<PCamera> omgMobileDataList = omgMobileData();

        List<PCamera> fliPCameraList = flipKartMobileData();

        List<PCamera> amazonResults = getAmazonResults();

       // List<PCamera> snapAndInfiResults = getSnapdealAndInfibeamResults();

        System.out.println("omgMobileDataList size : " + omgMobileDataList.size());

        for (PCamera product : omgMobileDataList) {
            dataServices.addPCameraEntity(product);
            System.out.println("cameracreate Product added Successfully !");

        }

        for (PCamera product : fliPCameraList) {
        ...
        for (PCamera product : amazonResults) {
            dataServices.addPCameraEntity(product);
            System.out.println("cameracreate Product added Successfully !");

        }

      /*  for (PCamera product : snapAndInfiResults) {
            dataServices.addPCameraEntity(product);
            System.out.println("cameracreate Product added Successfully !");

        }*/

        return new Status(1, "cameracreate Product added Successfully !");

    }

    public List<PCamera> omgMobileData() throws IOException, JSONException, org.json.JSONException {

        JSONObject jsonObject = null;
        String str = "http://feeds.omgeu.com/data/feed.aspx?hash=688d0b57a8624b1b8c018a2badf1fef4" + "&page=1";
        URL url = new URL(str);
        ...
        List<PCamera> productList = new ArrayList<PCamera>();
        for (int i = 0; i < productList1.length(); i++) {

            PCamera pinfo = new PCamera();
            ...
            if (prodAttr instanceof JSONArray) {
                ...
                for (int j = 0; j < ((JSONArray) prodAttr).length(); j++) {
                    ...
                    pinfo.setMediumImage(productList1.getJSONObject(i).getString("LargeImage"));
    ...
    public List<PCamera> flipKartMobileData() throws IOException, JSONException, org.json.JSONException {

        List<PCamera> productList = new ArrayList<PCamera>();

        HeaderColumnNameTranslateMappingStrategy<PCamera> beanStrategy = new HeaderColumnNameTranslateMappingStrategy<PCamera>();
        beanStrategy.setType(PCamera.class);
        ...
        CsvToBean<PCamera> csvToBean = new CsvToBean<PCamera>();
        CSVReader reader = new CSVReader(new FileReader("C:\\pingleCSV\\Camera\\flipkartCamera.csv"));
        List<PCamera> emps = csvToBean.parse(beanStrategy, reader);
        System.out.println(emps.size());
        for (PCamera product : emps) {
    ...
    public List<PCamera> getAmazonResults() {
        // Set the service:
        ...
        List<PCamera> products = new ArrayList<PCamera>();
        for (int i = 1; i < 10; i++) {


            // Fill in the request object:
            itemRequest.setSearchIndex("All");
            itemRequest.setKeywords("Camera");
...
*/
    ...
    /*public List<PCamera> getSnapdealAndInfibeamResults() throws FileNotFoundException {

        List<PCamera> productList = new ArrayList<PCamera>();

        HeaderColumnNameTranslateMappingStrategy<PCamera> beanStrategys = new HeaderColumnNameTranslateMappingStrategy<PCamera>();
        beanStrategys.setType(PCamera.class);
        ...
        CsvToBean<PCamera> csvToBeans = new CsvToBean<PCamera>();
        CSVReader readers = new CSVReader(new FileReader("C:\\pingleCSV\\Laptops\\SnapdealLaptops.csv"));
        List<PCamera> empss = csvToBeans.parse(beanStrategys, readers);
        System.out.println("omg list size = " + empss.size());
        for (PCamera product : empss) {
            // System.out.println("Name - " + product.getProductName() + "Price - " + product.getSalePrice() + " - color - " + product.getColor());
            List<String> items = Arrays.asList(product.getMediumImage().split("\\s*,\\s*"));
            product.setMediumImage(items.get(0));
      ...
      *//*  HeaderColumnNameTranslateMappingStrategy<PCamera> beanStrategy2 = new HeaderColumnNameTranslateMappingStrategy<PCamera>();
        beanStrategy2.setType(PCamera.class);
        ...
        CsvToBean<PCamera> csvToBean2 = new CsvToBean<PCamera>();
        CSVReader reader2 = new CSVReader(new FileReader("C:\\pingleCSV\\InfibeamMobilesFeed.csv"));
        List<PCamera> emps2 = csvToBean2.parse(beanStrategy2, reader2);
        System.out.println("omg list size = " + emps2.size());
        for (PCamera product : emps2) {
            // System.out.println("Name - " + product.getProductName() + "Price - " + product.getSalePrice() + " - color - " + product.getColor());
