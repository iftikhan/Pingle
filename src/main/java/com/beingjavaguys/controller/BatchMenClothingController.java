...
public class BatchMenClothingController {
    ...
    @RequestMapping(value = "/Menclothing", method = RequestMethod.POST, consumes = MediaType.ALL_VALUE)
    public
    @ResponseBody
    Status addMobile() throws Exception {
        System.out.println("Calling Menclothing Services");

        List<PMenClothing> finalList = new ArrayList<>();

        JSONObject jsonObject = null;
        String str = "http://feeds.omgeu.com/data/feed.aspx?hash=06f8c1524c0f46829a8f8c0bb565ed42" + "&page=1";
        URL url = new URL(str);
        ...
        for (int i = 1; i < 10; i++) {
            List<PMenClothing> list = new ArrayList<>();
        ...
        for (PMenClothing product : finalList) {
            if (product.getMediumImage()!=null) {
                dataServices.addPMenClothingEntity(product);
                Thread.sleep(50);
            }
        }

        // List<PMenClothing> fliPMenClothingList = flipKartMobileData();

        //  List<PMenClothing> amazonResults = getAmazonResults();

        // List<PMenClothing> snapAndInfiResults = getSnapdealAndInfibeamResults();

        //   System.out.println("omgMobileDataList size : " + omgMobileDataList.size());

     /*   for (PMenClothing product : omgMobileDataList) {
...
*/
        /*for (PMenClothing product : fliPMenClothingList) {
       ...
       /* for (PMenClothing product : amazonResults) {
...
*/
      /*  for (PMenClothing product : snapAndInfiResults) {
            dataServices.addPMenClothingEntity(product);
            System.out.println("tshirtcreate Product added Successfully !");
    ...
    public List<PMenClothing> omgMobileData(int pageNo) throws IOException, JSONException, org.json.JSONException {

        JSONObject jsonObject = null;
        String str = "http://feeds.omgeu.com/data/feed.aspx?hash=06f8c1524c0f46829a8f8c0bb565ed42" + "&page=" + pageNo;
        URL url = new URL(str);
        InputStream is = url.openStream();
        int ptr = 0;
        StringBuilder builder = new StringBuilder();
        ...
        List<PMenClothing> productList = new ArrayList<PMenClothing>();
        for (int i = 0; i < productList1.length(); i++) {

            PMenClothing pinfo = new PMenClothing();
    ...
    public List<PMenClothing> flipKartMobileData() throws IOException, JSONException, org.json.JSONException {

        List<PMenClothing> productList = new ArrayList<PMenClothing>();

        HeaderColumnNameTranslateMappingStrategy<PMenClothing> beanStrategy = new HeaderColumnNameTranslateMappingStrategy<PMenClothing>();
        beanStrategy.setType(PMenClothing.class);
        ...
        CsvToBean<PMenClothing> csvToBean = new CsvToBean<PMenClothing>();
        CSVReader reader = new CSVReader(new FileReader("C:\\pingleCSV\\MenClothing\\MenClothing8Oct.csv"));
        List<PMenClothing> emps = csvToBean.parse(beanStrategy, reader);
        // System.out.println(emps.size());
        for (PMenClothing product : emps) {
  ...
  /*  public List<PMenClothing> getAmazonResults() {
        // Set the service:
        ...
        List<PMenClothing> products = new ArrayList<PMenClothing>();
        for (int i = 1; i < 10; i++) {
            ...
            for (Items item : response.getItems()) {
                ...
                for (Item item1 : item.getItem()) {
                    PMenClothing product = new PMenClothing();
...
*/
    ...
    /*public List<PMenClothing> getSnapdealAndInfibeamResults() throws FileNotFoundException {

        List<PMenClothing> productList = new ArrayList<PMenClothing>();

        HeaderColumnNameTranslateMappingStrategy<PMenClothing> beanStrategys = new HeaderColumnNameTranslateMappingStrategy<PMenClothing>();
        beanStrategys.setType(PMenClothing.class);
        ...
        CsvToBean<PMenClothing> csvToBeans = new CsvToBean<PMenClothing>();
        CSVReader readers = new CSVReader(new FileReader("C:\\pingleCSV\\Laptops\\SnapdealLaptops.csv"));
        List<PMenClothing> empss = csvToBeans.parse(beanStrategys, readers);
        System.out.println("omg list size = " + empss.size());
        for (PMenClothing product : empss) {
            // System.out.println("Name - " + product.getProductName() + "Price - " + product.getSalePrice() + " - color - " + product.getColor());
            List<String> items = Arrays.asList(product.getMediumImage().split("\\s*,\\s*"));
            product.setMediumImage(items.get(0));
      ...
      *//*  HeaderColumnNameTranslateMappingStrategy<PMenClothing> beanStrategy2 = new HeaderColumnNameTranslateMappingStrategy<PMenClothing>();
        beanStrategy2.setType(PMenClothing.class);
        ...
        CsvToBean<PMenClothing> csvToBean2 = new CsvToBean<PMenClothing>();
        CSVReader reader2 = new CSVReader(new FileReader("C:\\pingleCSV\\InfibeamMobilesFeed.csv"));
        List<PMenClothing> emps2 = csvToBean2.parse(beanStrategy2, reader2);
        System.out.println("omg list size = " + emps2.size());
        for (PMenClothing product : emps2) {
            // System.out.println("Name - " + product.getProductName() + "Price - " + product.getSalePrice() + " - color - " + product.getColor());
